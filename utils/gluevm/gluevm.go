...
var remote_host_ip string = "10.10.5.1"
var remote_root_rsa_id_path string = "/root/.ssh/id_rsa"

func VmState() (output string, err error) {
...
func VmStart() (output string, err error) {

	var stdoutVmStart []byte

	if gin.IsDebugging() == true {
		//  For Remote
		client, err_val := utils.ConnectSSH(remote_host_ip, remote_root_rsa_id_path)
		if err_val != nil {
			err = err_val
		...
		stdoutVmStart = []byte("{\"code\": 200, \"val\": \"Gateway VM Create Success\", \"name\": \"create\", \"type\": \"str\"}")
	}
	output = string(stdoutVmStart)
	return
}

func VmStop() (output string, err error) {
	...
	if gin.IsDebugging() == true {
		...
		stdoutVmStop = []byte("{\"code\": 200, \"val\": \"Gateway VM Create Success\", \"name\": \"create\", \"type\": \"str\"}")
	}
...
func VmDelete() (output string, err error) {
	...
	if gin.IsDebugging() == true {
		...
		stdoutVmDelete = []byte("{\"code\": 200, \"val\": \"Gateway VM Create Success\", \"name\": \"create\", \"type\": \"str\"}")
	}
...
func VmCleanup() (output string, err error) {
	...
	if gin.IsDebugging() == true {
		...
		stdoutVmCleanup = []byte("{\"code\": 200, \"val\": \"Gateway VM Create Success\", \"name\": \"create\", \"type\": \"str\"}")
	}
...
func VmMigrate(target string) (output string, err error) {
	...
	if gin.IsDebugging() == true {
		...
		stdoutVmMigrate = []byte("{\"code\": 200, \"val\": \"Gateway VM Create Success\", \"name\": \"create\", \"type\": \"str\"}")
	}
